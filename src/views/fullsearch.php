...
<body>
	...
	(function() {
		...
		// Resolve language codes to display names using the browser's Intl API
		...
		function buildSearchUrl(query, page) {
			const params = new URLSearchParams({
				q: query || '*',
				query_by: 'title,artist,album,composer,lyrics,comment',
				per_page: '20',
				page: String(page),
				facet_by: 'artist,album,language',
			});
		...
		function renderResults(data) {
			...
			data.hits.forEach(function(hit) {
				...
				if (doc.language && doc.language.length > 0) {
					meta.appendChild(createChip('language', doc.language.map(getLanguageName).join(', ')));
				}
		...
		function renderFacets(facetCounts) {
			...
			facetCounts.forEach(function(facet) {
				...
				facet.counts.slice(0, 5).forEach(function(count) {
					...
					const displayValue = facet.field_name === 'language' ? getLanguageName(count.value) : count.value;
					btn.textContent = displayValue + ' (' + count.count + ')';
